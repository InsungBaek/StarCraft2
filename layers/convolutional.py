...
class VggBlock3D(nn.Module):
    """Add class docstring."""
    def __init__(self, in_channels, out_channels, stride=1, downsample=None, layer_count=2):
        ...
        self.conv1 = nn.Conv3d(self.in_ch, self.out_ch, 3, 1, padding=1, bias=False)
        self.conv2 = nn.Conv3d(self.out_ch, self.out_ch, 3, 1, padding=1, bias=False)
        self.conv3 = nn.Conv3d(self.out_ch, self.out_ch, 3, 1, padding=1, bias=False)
        self.pool = nn.MaxPool3d(kernel_size=(2, 2, 2), stride=1, padding=1)        
